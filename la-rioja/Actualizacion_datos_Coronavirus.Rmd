...
## Casos confirmados por PCR en las residencias de personas mayores de La Rioja

Casos de COVID-19 confirmados mediante prueba PCR en las residencias de personas mayores de La Rioja

La página de datos abiertos es esta: [https://web.larioja.org/dato-abierto/datoabierto?n=opd-471](https://web.larioja.org/dato-abierto/datoabierto?n=opd-471)

```{r}
page_confirmados_pcr_residencias <- read_html('https://web.larioja.org/dato-abierto/datoabierto?n=opd-471')

# find all nodes with a class of "col_md_6"
col_md_6_confirmados_pcr_residencias <- html_nodes(page_confirmados_pcr_residencias, css = '.col-md-6')
col_md_6_confirmados_pcr_residencias
```

```{r}
fecha_campos_confirmados_pcr_residencias <- html_children(col_md_6_confirmados_pcr_residencias[8]) %>% 
    html_nodes("dt") %>% 
    html_text(trim = TRUE)
fecha_campos_confirmados_pcr_residencias
```

```{r}
fecha_valores_confirmados_pcr_residencias <- html_children(col_md_6_confirmados_pcr_residencias[8]) %>% 
    html_nodes("dd") %>% 
    html_text(trim = TRUE)
fecha_valores_confirmados_pcr_residencias
```

Ahora bajamos el fichero

```{r}
link_confirmados_pcr_residencias <- 'https://ias1.larioja.org/opendata/download?r=Y2Q9NDcxfGNmPTAz'
```


```{r}
rioja_confirmados_pcr_residencias <- read_delim(link_confirmados_pcr_residencias, 
                                                ",", escape_double = FALSE,                                
                                                na = c("", "NA", "-"), 
                                                col_names = TRUE, trim_ws = TRUE)      # OJO con el col_names ************************************
```


```{r, eval=FALSE}
httr::GET(link_confirmados_pcr_residencias, httr::write_disk(tf_confirmados_pcr_residencias <- tempfile(fileext = ".csv")))

rioja_confirmados_pcr_residencias <- read_delim(tf_confirmados_pcr_residencias, 
                                                ",", escape_double = FALSE,                                
                                                na = c("", "NA", "-"), 
...
```{r}
rioja_confirmados_pcr_residencias <- as.data.frame(rioja_confirmados_pcr_residencias)
```

```{r}
names(rioja_confirmados_pcr_residencias) <- c("residencias", "municipio", 
                                              "confirmados_pcr")
```

```{r, eval=FALSE}
rioja_confirmados_pcr_residencias <- rioja_confirmados_pcr_residencias[, 1:3]
```

Incorporamos la fecha del dato

```{r}
rioja_confirmados_pcr_residencias$fecha_publicacion_confirmados_pcr_residencias    = as.Date(fecha_valores_confirmados_pcr_residencias[1], '%d/%m/%Y')
rioja_confirmados_pcr_residencias$ultima_actualizacion_confirmados_pcr_residencias = as.Date(fecha_valores_confirmados_pcr_residencias[2], '%d/%m/%Y')
rioja_confirmados_pcr_residencias$ambito_temporal_confirmados_pcr_residencias      = as.Date(fecha_valores_confirmados_pcr_residencias[3], '%d/%m/%Y')
```

```{r}
summary(rioja_confirmados_pcr_residencias)
```

```{r}
str(rioja_confirmados_pcr_residencias)
```
...
diario <- data.frame(
                     ...
                     mayores_confirmados_PCR       = as.numeric(rioja_confirmados_pcr_residencias$X2[1]),
                     mayores_altas_acumulado       = as.numeric(rioja_confirmados_pcr_residencias$X2[2]),
                     mayores_fallecidos_acumulado  = as.numeric(rioja_confirmados_pcr_residencias$X2[3]),
                     residentes_sin_coronavirus    = rioja_confirmados_pcr_residencias$X2[4],
                     residencias_sin_positivos     = as.numeric(rioja_confirmados_pcr_residencias$X2[5]),
                     
                     fecha_publicacion_confirmados_pcr_residencias       = as.Date(fecha_valores_confirmados_pcr_residencias[1], '%d/%m/%Y'),
                     ultima_actualizacion_confirmados_pcr_residencias    = as.Date(fecha_valores_confirmados_pcr_residencias[2], '%d/%m/%Y'),
                     ambito_temporal_confirmados_pcr_residencias         = as.Date(fecha_valores_confirmados_pcr_residencias[3], '%d/%m/%Y')
                     )
```
...
```{r}
saveRDS(rioja_confirmados_pcr_residencias, file = paste0('./data/rioja_confirmados_pcr_residencias_', Sys.Date(),'.rds'))
```
